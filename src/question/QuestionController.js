...
router.get("/", VerifyToken, function(req, res) {
    Question.find({}, function(err, questions) {
        if (err)
            return res
                .status(500)
                .send("There was a problem finding the questions.");
        if (questions) {
            res.status(200).send(
...
router.get("/answer/:id", VerifyToken, function(req, res) {
    Question.findById(req.params.id, function(err, question) {
        if (err)
            return res
                .status(500)
                .send("There was a problem finding the answer.");
...
router.delete("/:id", function(req, res) {
    Question.findByIdAndRemove(req.params.id, function(err, question) {
        if (err)
            return res
                .status(500)
                .send("There was a problem deleting the question.");
